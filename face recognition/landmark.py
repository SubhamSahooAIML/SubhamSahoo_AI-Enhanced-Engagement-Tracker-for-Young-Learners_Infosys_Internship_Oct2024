...
screenshot_dir = "C:\\Users\\Acer\\OneDrive\\Desktop\\attention_score\\screenshots_landmark"
...
try:
    while True:
        ...
        frame = cv.resize(frame, ( 740,480))  # Downscale for faster processing
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = os.path.join(screenshot_dir, f"{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg")
                if attentive:
        ...
        # Display the frame
        cv.imshow("Video Stream for landmark.py", frame)
    ...
    if not df.empty:
        df.to_excel('C:\\Users\\Acer\\OneDrive\\Desktop\\attention_score\\attendance_with_attention_landmark.xlsx', index=False)
        print("Attendance with attentiveness saved to 'attendance_with_attention_landmark.xlsx'.")
