...
while True:
    ...
    frame = cv.resize(frame, (720, 480))  # Adjust resolution as needed
    ...
    for face_encoding in face_encodings:
        ...
        if distance < confidence_threshold:
            ...
            for face_location in face_locations:
                ...
                cv.putText(frame, 'SUBHAM ', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
            ...
            new_entry = pd.DataFrame({'Name': ['SUBHAM SAHOO'], 'Date': [date_str], 'Time': [time_str]})
    ...
    if recognition_count >= 5:
        ...
        df.to_excel("C:\\Users\Acer\\OneDrive\\Desktop\\attention_score\\recognized_faces_SUBHAM_for tools.py.xlsx", index=False)
        print("Excel file created successfully.")
        recognition_count = 0  # Reset the counter
        df = pd.DataFrame(columns=columns)  # Reset the DataFrame

    cv.imshow('Video Stream tools.py', frame)
