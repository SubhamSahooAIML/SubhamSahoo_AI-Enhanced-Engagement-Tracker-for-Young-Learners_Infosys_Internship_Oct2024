    ...
    for face_encoding in face_encodings:
        ...
        if distance < confidence_threshold:  # Check if the distance is below the threshold
            ...
            for face_location in face_locations:
                ...
                frame = cv.putText(frame, 'SUBHAM SAHOO', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                                   (255, 0, 0), 2, cv.LINE_AA)

    if not recognized:
        frame = cv.putText(frame, 'Not SUBHAM SAHOO', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                           (255, 0, 0), 2, cv.LINE_AA)

    # Display the resulting frame
    cv.imshow('Video Stream for face_Recog.py', frame)
